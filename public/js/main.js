...
$(document).ready(function () {
    ...
    $('.default-submit').on('click', function (event) {
        ...
        {
            localStorage.setItem('tz_section',section_id);
            $.post("tz_mail/personal-contact.php", {
...
/*==============================================================
 ...
 var words = [
  { weight: 8, text: 'Ruby' },
  { weight: 7, text: 'JavaScript' },
  { weight: 7, text: 'Rails' },
  { weight: 4, text: 'Angular' },
  { weight: 5, text: 'Bootstrap' },
  { weight: 3, text: 'Sass' },
  { weight: 4, text: 'Haml' },
  { weight: 2, text: 'CoffeScript' },
  { weight: 7, text: 'MySQL' },
  { weight: 5, text: 'Postgres' },
  { weight: 7, text: 'Git' },
  { weight: 2, text: 'Vim' },
  { weight: 6, text: 'Atom' },
  { weight: 3, text: 'Docker' },
  { weight: 8, text: 'Zendesk' },
  { weight: 8, text: 'Github' },
  { weight: 2, text: 'Amazon EC2' },
  { weight: 7, text: 'Responsive Design' },
  { weight: 8, text: 'jQuery' },
  { weight: 6, text: 'Ajax' },
  { weight: 7, text: 'Heroku' },
  { weight: 7, text: 'Linux SysAdmin' },
  { weight: 8, text: 'Ubuntu' },
  { weight: 6, text: 'REST APIs' },
  { weight: 7, text: 'Sinatra' },
  { weight: 6, text: 'Handlebars' },
  { weight: 6, text: 'GitLab' }
 ];
 _.map(words, function(word){
   word.html = { class: _.sample(animateEffects) };
   return word;
 });
 var baseColor = $('.text-puerto-rico').css('color');
 var gradient = tinygradient([
   baseColor,
   tinycolor(baseColor).lighten(25).desaturate(25).toHexString()
 ]);
 $('.skills-word-cloud').jQCloud(words, {
   autoResize: true,
   colors: _.map(gradient.hsv(10), function(color) {
     return color.toHexString();
   }),
   classPattern: 'w{n} alt-font wow',
   fontSize: function(containerWidth, containerHeight, step) {
     var b = containerWidth > 775 ? 20 : 8;
     return (2.5 * step + b);
   }
 });
...
//Replaces inline script template with content
var replaceTemplate = function($source, options={}) {
  var template = Handlebars.compile($source.html());
  $source.replaceWith(template(options));
};

var skills = [
  { name: "Ruby", percent: 85, exp: { quantity: 4, unit: 'years'} },
...
var socialIcons = [
  ...
  { fa_class: 'fa-stack-overflow', link: 'http://stackoverflow.com/users/3376769/jpalmieri'},
